...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            losses, predicts = model(trains, labels, mode="train")
            model.zero_grad()
            ...
            optimizer.step()
            if total_batch % 10 == 0:
                dev_acc_lev1, dev_acc_lev2, dev_loss = evaluate(config, model, dev_iter)
...
def test(config, model, test_iter):
    ...
    model.eval()
    start_time = time.time()
    test_acc_lev1, test_acc_lev2, test_loss, test_report, test_confusion = evaluate(config, model, test_iter, test=True)
...
def evaluate(config, model, data_iter, test=False):
    ...
    with torch.no_grad():
        for texts, labels in data_iter:
            losses, predicts = model(texts, labels, "evl")
            loss = torch.mean(torch.stack(losses))
            loss_total += loss.item()

            true = labels.data.cpu()
            predicts_lev1 = [r[0].detach().cpu().numpy() for r in predicts]
            predic_lev1_index = [torch.max(torch.tensor(r).data, -1)[1].cpu() for r in predicts_lev1]
            predicts_lev2 = [r[t + 1].detach().cpu().numpy() for r, t in zip(predicts, predic_lev1_index)]
...
if __name__ == '__main__':
    dataset = '/data1/sina_dw/dandan33/Bert-Chinese-Text-Classification-Pytorch-master'  # 数据集
    ...
    train(config, model, train_iter, dev_iter, train_iter)
